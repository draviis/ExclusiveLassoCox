...
#'    \item \code{lmd}: The log of the regularization parameter \eqn{\lmd}{lmd}.
...
#' plot(exfit)
#' plot(exfit, legend=NULL, xvar="lmd")
plot.ExclusiveLassoFit <- function(x, xvar=c("norm", "lmd", "l1"),
...
plot.ExclusiveLassoFit_cv <- function(x, bar.width=0.01, ...){
    ...
    log_lmd <- log(x$lmd)
    bar.width <- bar.width * diff(range(log_lmd))
    ...
    points(log_lmd, x$cvm, pch=20, col="red")

    segments(log_lmd, x$cvlo,
             log_lmd, x$cvup,
             col="darkgrey")
    segments(log_lmd - bar.width, x$cvlo,
             log_lmd + bar.width, x$cvlo,
             col="darkgrey")
    segments(log_lmd - bar.width, x$cvup,
             log_lmd + bar.width, x$cvup,
             col="darkgrey")

    abline(v=log(x$lmd.min), lty = 3, lwd = 2)
    abline(v=log(x$lmd.1se), lty = 3, lwd = 2)
}
